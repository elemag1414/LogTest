...
function Sidebar({ updateHandler, smooth, setSmooth, data, showModal }) {
    ...
    const buttonHandler = (val) => {
        ...
        fetch(`/api/data/new_fetch`)            // new_fetch request (request to update all log data)
            .then(response => response.json())  // 여기서 반환된 Promise를 다음 then에서 처리합니다.
            .then(data => {
                console.log("new_fetch result: ", data);
